...
const resetSectionOfPage = async (): Promise<object> => {
  try {
    const payload = await client.section_of_page.deleteMany({});
    return {
      status: true,
      message: 'reset section of page by id',
      payload
    };
  } catch (e) {
    return {
      status: false,
      message: 'Failed reset section of page by id',
      payload: true
    };
  }
};
