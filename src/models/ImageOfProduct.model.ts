...
const getHotProduct = async (limit: number): Promise<object> => {
  try {
    const payload = await prisma.image_of_product.findMany({
      include: {
        product: {
          select: {
            name: true,
...
module.exports = { createImageOfProduct, findImageOfProductById, deleteImageOfProduct,getHotProduct };
